...
// array() : 파일 여러 개 업로드 (name(클라이언트 입장에선 input!) 하나로 여러 개의 파일을 받는 방법)
// req.files 생성
app.post('/upload/array', uploadDetail.array('userfile'), function (req, res) {
  console.log('file 여러개(v1) : ', req.files);
  res.send('여러개 업로드 성공(v1)');
});
...
app.post(
  ...
  function (req, res) {
    console.log('file 여러개(v2) : ', req.files);
    console.log('req.body :', req.body);
    res.send('여러개 업로드 성공(v2)');
  }
);

// 결과값으로 클라이언트가 업로드한 이미지를 보여주려면?? 미들웨어도 적어줘야함(본 문서 10번코드)
app.post(
  '/upload/detail/v1',
  uploadDetail.single('userfile'),
  function (req, res) {
